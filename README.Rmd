...
por meio da especificação `Swagger`, define uma estrutura de API a partir
de rotas, facilitando a implementação e a verificação com testes
para validar o comportamento das mesmas.
...
Em certos casos, é preferível excluir uma sequência de observações, por meio da sequência `1:3`, cuja requisição é dada por `/data/delete_row?ID=1%3A4`.
...
Além disso, essa rota pode retornar mais de uma predição, caso a requisição seja, por exemplo, `x = 10, 20` e `grupo = B, A`, então a requisição é dada por `/fit/pred?x=10%2C20&grupo=B%2CA`.
...
Considere que foi necessário adicionar mais observações para a análise gráfica da regressão. 
